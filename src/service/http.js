...
export default {
  ...
  orderDetails(order_id) {
    return fetch('orderDetails', { order_id });
  },
  /** 修改派工状态
     * @order_id 工单id
     * @is_dispatch 是否派工
  ...
  /** 获取获取钉钉信息 */
  getConfig(data) {
    return fetch('https://dni.cel-cat.com.cn/xcadmin/public/index.php/woapi/getConfig', data);
  },
  /** 获取获取Token */
  getToken(code) {
  ...
  workRecordDetails(data) {
    let a = data;
    if (!a) {
      a = JSON.parse(sessionStorage.getItem('cardItem'));
    } else {
      sessionStorage.setItem('cardItem', JSON.stringify(a));
    }
    return fetch('workRecordDetails', { is_copy: 0, ...a });
  },
  /**
    * 工卡删除
    */
  ...
  sos(data) {
    return fetch('sos', {
      part_id: 0, page: 1, pagesize: 100, ...data,
    });
  },
  /**
    * 部件类型获取
  ...
  /**
    * 获取阿里云配置
    */
  getOssConfig(type = 'workorder') {
    return fetch('https://dni.cel-cat.com.cn/xcadmin/public/index.php/workorder/getOssConfig', { type });
  },
  /**
    * 工单现场发现修改
    */
