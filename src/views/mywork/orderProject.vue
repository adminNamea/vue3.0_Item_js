<template>
  <div>
    ...
    <card class="station" :top="false" :hed="false">
      ...
      <van-cell
        :title="filterType(stationList.order_type)"
        @click="toReport(stationList.order_type)"
      ...
      <van-cell
        title="费用添加"
        value
        @click="$router.push({ name: 'addFees' })"
        is-link
      />
    </card>
    <card class="station" :top="false" :hed="false">
...
export default {
  ...
  created() {
    if (this.$route.params.order_item_id) {
      sessionStorage.setItem("order_item_id", this.$route.params.order_item_id);
    }
